#ifndef _LEDraw_palettes_h
#define _LEDraw_palettes_h
//#pragma ones
#include <stdint.h>
#include <FastLED.h>
#include <colorutils.h>

// LEDraw! Fast Drawing Library of Palettes access for FastLED by PalPalych
// v0.0	- Eurica by PalPalych 23.09.2020
// v1.0	- Convert from Guyver garland by PalPalych 17.10.2020
...
/*typedef struct
{
	const TProgmemRGBPalette16 *const *palettes;
	uint8_t size;
} paletteSet_t;*/

const TProgmemRGBGradientPalettePtr gradientPalettes[] = {
		&Sunset_Real_gp[0],							 //0
		&es_rivendell_15_gp[0],					 //1
		&es_ocean_breeze_036_gp[0],			 //2
		&rgi_15_gp[0],									 //3
		&retro2_16_gp[0],								 //4
		&Analogous_1_gp[0],							 //5
		&es_pinksplash_08_gp[0],				 //6
		&Coral_reef_gp[0],							 //7
																		 //
		&es_ocean_breeze_068_gp[0],			 //8
		&es_pinksplash_07_gp[0],				 //9
		&es_vintage_01_gp[0],						 //10
		&departure_gp[0],								 //11
		&es_landscape_64_gp[0],					 //12
		&es_landscape_33_gp[0],					 //13
		&rainbowsherbet_gp[0],					 //14
		&gr65_hult_gp[0],								 //15
																		 //
		&gr64_hult_gp[0],								 //16
		&GMT_drywet_gp[0],							 //17
		&ib_jul01_gp[0],								 //18
		&es_vintage_57_gp[0],						 //29
		&ib15_gp[0],										 //20
		&Fuschia_7_gp[0],								 //21
		&es_emerald_dragon_08_gp[0],		 //22
		&lava_gp[0],										 //23
																		 //
		&fire_gp[0],										 //24
		&Colorfull_gp[0],								 //25
		&Magenta_Evening_gp[0],					 //26
		&Pink_Purple_gp[0],							 //27
		&es_autumn_19_gp[0],						 //28
		&BlacK_Blue_Magenta_White_gp[0], //29
		&BlacK_Magenta_Red_gp[0],				 //30
		&BlacK_Red_Magenta_Yellow_gp[0], //31
																		 //
		&Blue_Cyan_Yellow_gp[0],				 //32
		&bhw1_28_gp[0]};								 //33
...
typedef const TProgmemRGBPalette16 *TProgmemRGBPalette16Ptr;

// *** Classical FastLED Palettes Set ***
static TProgmemRGBPalette16Ptr palettes_fastled[] = {&RainbowColors_p, &OceanColors_p, &HeatColors_p, &PartyColors_p,
																										 &CloudColors_p, &LavaColors_p, &ForestColors_p, &RainbowStripeColors_p};

//static const TProgmemRGBPalette16 *palettes_fastled[] = {&RainbowColors_p, &OceanColors_p, &HeatColors_p, &PartyColors_p,
//																												 &CloudColors_p, &LavaColors_p, &ForestColors_p, &RainbowStripeColors_p};

// ----- PALETTES SETS SECTION -----
static paletteSet_t paletteSet[] = {
		{0, 0},
		{0, 0x80},
		{8, 0x80},
		{16, 0x80},
...
// return true value
LIB8STATIC uint8_t rescale8(uint8_t a, fract8 frac)
{
	return scale16(a * 256, frac);
}
...
CRGB ColorFromPalette(const TProgmemRGBGradientPalette_byte &progpal, uint8_t index, uint8_t brightness, TBlendType blendType)
{
	...
	{
		// uint8_t range = u1.index - u.index + 1U;			// rescale calculation
		// uint8_t amount = index - u.index;							// this calculation is not optimal by uses cpp division...
		// uint8_t f2 = (uint16_t)256U * amount / range; // but it allows you to achieve the implementation of this function
		// How it work:
		//           Gradient 1  			u.index								Grafient 2           u1.index  Gradient 3
		// |----------------------------|-----------------------x-------------------|----------------|
		// 0														60										index	= 142					192						   255
		//                              |<---Gradient amount--->| = 82
		//                              |<---------------Gradient range------------>| = 132
		// after calculation: Full_8bit_range * amount / range = 255 * 82 / 132 = 158 <- the needed index of blend
		uint8_t f2 = (uint16_t)255U * (index - u.index) / (u1.index - u.index + 1U);

		uint8_t f1 = ~f2;		 // <- more faster and more resource-intensive function than 255-f2 (a tiny cent saves a dollar)
		uint8_t red2 = u1.r; // vvv	then the repeated code from a similar function
